// !!!!!!!!!!!! использование boost/dynamic_bitset.hpp !!!!!!!!!!!!!!!!!!!!!!!!!!
// !!!!!!!!!!!!!!!!!! медленно !!!!!!!!!!!!!!!!!!!!!!!!!!
...
class Letter {
    public:
        ...
        BitLetter bits;
        bool operator==(const Letter& other) { return bits == other.bits; }
};
...
class Number {
    public:
        ...
        BitNumber bits;
        Number& operator++();
        bool operator==(const Number& other) { return bits == other.bits; }
...
std::vector<unsigned char> EncodedBitset::ConvertToChars() {
    ...
    while (bits.size() % (sizeof(unsigned char) * 8) != 0) {
        bits.push_back(0);
    }
    for (int i = 0; i < (bits.size() / (sizeof(unsigned char) * 8)); ++i) {
...
LZWTree::LZWTree() {
    Number number(0);
    Letter letter(' ');
    root = new Node({.number = number, letter = letter, .children = {}});
}
